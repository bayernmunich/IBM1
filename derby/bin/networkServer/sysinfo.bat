@REM ---------------------------------------------------------
@REM -- This batch file is an example of how to display Derby system information.
@REM -- this batch gets the sysinfo using default settings
@REM -- for non default behavior, one should use the networkServerControl.bat
@REM ---------------------------------------------------------
@echo off
...
"%JAVA_HOME%/bin/java" %DERBY_OPTS% -classpath "%DERBY_HOME%\lib\derbyclient.jar;%DERBY_HOME%\lib\derby.jar;%DERBY_HOME%\lib\derbytools.jar;%DERBY_HOME%\lib\jh.jar;%DERBY_HOME%\lib\derbynet.jar;%__DERBY_LOCALE_JARS_PATH__%" -Dderby.system.home="%DERBY_HOME%" org.apache.derby.drda.NetworkServerControl sysinfo %*
