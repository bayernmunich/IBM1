...
:runcmd
set CLASSPATH=%WAS_CLASSPATH%;%WAS_HOME%\properties;%WAS_HOME%\derby\lib\derby.jar;%WAS_HOME%\derby\migration\migratetoderby.jar
set PERFJAVAOPTION=-Xms512m -Xmx768m -Xj9 -Xquickstart
@echo off
...
set TMPPROP=%USER_INSTALL_ROOT:\=\\%\\properties\\ssl.client.props
>> %TMPJAVAPROPFILE% echo com.ibm.SSL.ConfigURL=file:%TMPPROP:"=%
set TMPPROP=%USER_INSTALL_ROOT:\=\\%
>> %TMPJAVAPROPFILE% echo com.ibm.ws.migration.currentProfileLogLocation=%TMPPROP:"=%
>> %TMPJAVAPROPFILE% echo KeepProfileName=true
...
"%JAVA_HOME%\bin\java" -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% %PERFJAVAOPTION% %WAS_LOGGING% %javaoption% %CONSOLE_ENCODING% "-Dwas.install.root=%WAS_HOME%" "-Duser.install.root=%USER_INSTALL_ROOT%" com.ibm.wsspi.bootstrap.WSPreLauncher -nosplash -application com.ibm.ws.bootstrap.WSLauncher com.ibm.ws.migration.WASPostUpgradeBLAHelper %*
set RC=%ERRORLEVEL%
