...
@REM JobManager Database Restore Tool

@REM Arguments:
@REM
@REM location - location of the backup database e.g c:/foo/bar also includes the database name in the string.
...
set DERBY_JAR="%DERBY_HOME%\lib\derby.jar"
set CLASSPATH=%WAS_CLASSPATH%;%DERBY_JAR%

"%JAVA_HOME%\bin\java" -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -cp %CLASSPATH% -Dcmd.properties.file=%TMPJAVAPROPFILE% %WAS_DEBUG% %WAS_TRACE% %CONSOLE_ENCODING% "%CLIENTSOAP%" "%JAASSOAP%" "%CLIENTSAS%" "%CLIENTSSL%"  %USER_INSTALL_PROP% "-Dwas.install.root=%WAS_HOME%" "-DWAS_HOME=%WAS_HOME%" "com.ibm.wsspi.bootstrap.WSPreLauncher" -nosplash -application "com.ibm.ws.bootstrap.WSLauncher"  "com.ibm.ws.admin.system.JobManagerRestore" "%USER_INSTALL_ROOT%" %*
set RC=%ERRORLEVEL%
