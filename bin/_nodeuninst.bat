...
@REM Node Uninstall Utility

@REM *******************************************************************
@REM * IMPORTANT:
@REM * This tool is meant to be run only during uninstall. It is not
@REM * meant for customer use. To uninstall WebSphere, use the
@REM * WebSphere uninstall program. To remove a node from a cell, use
@REM * the removeNode program.
@REM *******************************************************************
...
%JAVA_EXE% -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% %WAS_DEBUG% %WAS_TRACE% %CONSOLE_ENCODING% "%CLIENTSOAP%" "%JAASSOAP%" "%CLIENTSAS%" "%CLIENTSSL%" %USER_INSTALL_PROP% "-Dwas.install.root=%WAS_HOME%" "com.ibm.wsspi.bootstrap.WSPreLauncher" -nosplash -application "com.ibm.ws.bootstrap.WSLauncher"   "com.ibm.ws.admin.services.NodeUninstallPrep" "%CONFIG_ROOT%" "%WAS_CELL%" "%WAS_NODE%" %*
set RC=%ERRORLEVEL%
