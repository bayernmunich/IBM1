...
"%JAVA_HOME%\bin\java" -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% %WAS_LOGGING% %CONSOLE_ENCODING% -DKeepProfileName=true -Xbootclasspath/p:"%WAS_BOOTCLASSPATH%" -DWAS_HOME="%WAS_HOME%" -Dcom.ibm.websphere.migration.serverRoot="%WAS_HOME%" "-Dwas.install.root=%WAS_HOME%" "-Duser.install.root=%USER_INSTALL_ROOT%" com.ibm.wsspi.bootstrap.WSPreLauncher -nosplash -application com.ibm.ws.bootstrap.WSLauncher com.ibm.ws.migration.ClientUpgrade %*
set RC=%ERRORLEVEL%
