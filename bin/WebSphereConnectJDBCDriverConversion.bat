...
"%JAVA_HOME%\bin\java" -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% %WAS_LOGGING% %CONSOLE_ENCODING% -Xbootclasspath/p:"%WAS_BOOTCLASSPATH%" "-Dwas.install.root=%WAS_HOME%" "-Duser.install.root=%USER_INSTALL_ROOT%" "-Dosgi.migration.plugin.name=com.ibm.wsspi.extension.was-migration-ddconversiontool" com.ibm.wsspi.bootstrap.WSPreLauncher -nosplash  -application com.ibm.ws.bootstrap.WSLauncher com.ibm.ws.migration.ConversionTool %*
set RC=%ERRORLEVEL%
