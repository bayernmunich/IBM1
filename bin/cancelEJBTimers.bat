...
@REM EJB Timer Service Cancel EJB Timers Command Launcher

@REM Launch Arguments:
@REM
@REM cancelEJBTimers <server> <filter> [options]
@REM server  : the name of the server process to look for timers.
@REM filter: <-all> | <-timer timerid> | <-app [-mod [-bean]]>
@REM         -all                        : cancel all timers
@REM         -timer <timerid>            : cancel a specific timer only
@REM         -app <application name>     : cancel all timers for an applicaiton
@REM         -mod <module name>          : cancel all timers for a module
@REM         -bean <bean name>           : cancel all timers for an EJB type
@REM options:
@REM         -host <host name>           : host name to connect to for command
@REM         -port <port number>         : port number to connect to for command
@REM         -conntype <connector type>  : connection type (SOAP, RMI, NONE)
@REM         -user <userid>              : user name / idintifier
@REM         -password <password>        : user password
@REM         -quiet                      : disables output
@REM         -logfile <filename>         : directs output to a file
@REM         -replacelog                 : clears the log file prior to executing
@REM         -trace                      : enables trace
@REM         -help                       : provides command specific help
@REM         -?                          : provides command specific help
...
@REM For debugging the cancel program itself
...
%JAVA_EXE% -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% "%CLIENTSOAP%" "%JAASSOAP%" "%CLIENTSAS%" "%CLIENTSSL%" %WAS_TRACE% %WAS_DEBUG% %CONSOLE_ENCODING% %USER_INSTALL_PROP% "-Dwas.install.root=%WAS_HOME%" "com.ibm.wsspi.bootstrap.WSPreLauncher" -nosplash -application "com.ibm.ws.bootstrap.WSLauncher" "com.ibm.ws.admin.services.CancelEJBTimersCommand" "%CONFIG_ROOT%" "%WAS_CELL%" "%WAS_NODE%" %*
set RC=%ERRORLEVEL%
