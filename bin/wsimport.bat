...
set WASFP_CLASSPATH=%WAS_HOME%\plugins\*;%WAS_CLASSPATH%
...
if defined CLIENTSSL set CLIENTSSL="%CLIENTSSL%"
"%JAVA_HOME%\bin\java" -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" %CLIENTSSL% %OSGI_INSTALL% %OSGI_CFG% %WAS_LOGGING% %CONSOLE_ENCODING% -Dwas.install.root="%WAS_HOME%" -Dws.ext.dirs="%WAS_EXT_DIRS%" %USER_INSTALL_PROP% -Djavax.xml.accessExternalSchema=all -Xbootclasspath/p:"%WAS_BOOTCLASSPATH%" -classpath "%WASFP_CLASSPATH%" com.ibm.ws.bootstrap.WSLauncher com.ibm.ws.jaxws.tools.WsImport %*
set RC=%ERRORLEVEL%
