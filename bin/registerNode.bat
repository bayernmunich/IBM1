...
@REM Profile Registration Tool

@REM Arguments:
@REM
@REM host - local host or AdminAgent host
@REM port - JMX port number for AdminAgent
@REM connType - Connection type use to connect to AdminAgent
...
%JAVA_EXE% -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% %WAS_DEBUG% %WAS_TRACE% %CONSOLE_ENCODING% "%CLIENTSOAP%" "%JAASSOAP%" "%CLIENTSAS%" "%CLIENTSSL%"  %USER_INSTALL_PROP% "-Dwas.install.root=%WAS_HOME%" "-DWAS_HOME=%WAS_HOME%" "com.ibm.wsspi.bootstrap.WSPreLauncher" -nosplash -application "com.ibm.ws.bootstrap.WSLauncher"  "com.ibm.ws.admin.services.ProfileRegistrationUtility" "%CONFIG_ROOT%" "%WAS_CELL%" "%WAS_NODE%" %*
set RC=%ERRORLEVEL%
