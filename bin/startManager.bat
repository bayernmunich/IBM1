...
@REM Configuration Based Deployment Manager Launcher
...
>> %TMPJAVAPROPFILE% echo osgi.configuration.area=%USER_INSTALL_ROOT%\servers\dmgr

set CLASSPATH=%WAS_CLASSPATH%

"%JAVA_HOME%\bin\java" "-Djava.endorsed.dirs=%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% %WAS_TRACE% %WAS_DEBUG% %CONSOLE_ENCODING% "%CLIENTSAS%" "%CLIENTSSL%" %USER_INSTALL_PROP% "-Dwas.install.root=%WAS_HOME%" com.ibm.ws.bootstrap.WSLauncher com.ibm.ws.management.tools.WsServerLauncher "%CONFIG_ROOT%" "%WAS_CELL%" "%WAS_NODE%" dmgr -dmgr %* %WORKSPACE_ROOT_PROP%
set RC=%ERRORLEVEL%
