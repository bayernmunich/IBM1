...
@REM
@REM serverName     - the name of the server process to be launched.
@REM -script [script_file_name] 
@REM -binaryData 
@REM -nowait 
@REM -quiet 
@REM -trace 
...
SET PATH=%WAS_PATH%
IF DEFINED WAS_DB2_PATH_VAR SET PATH=%WAS_DB2_PATH_VAR%;%PATH%
...
>> %TMPJAVAPROPFILE% echo osgi.configuration.area=%USER_INSTALL_ROOT%\servers\%1

set CLASSPATH=%WAS_CLASSPATH%

%JAVA_EXE% -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% %WAS_TRACE% %WAS_DEBUG% %CONSOLE_ENCODING% "%CLIENTSAS%" "%CLIENTSSL%" %USER_INSTALL_PROP% "-Dwas.install.root=%WAS_HOME%" com.ibm.ws.bootstrap.WSLauncher com.ibm.ws.management.tools.WsServerLauncher "%CONFIG_ROOT%" "%WAS_CELL%" "%WAS_NODE%" %* %WORKSPACE_ROOT_PROP%
SET RC=%ERRORLEVEL%
