...
"%JAVA_HOME%\bin\java" -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% "%CLIENTSOAP%" "%JAASSOAP%" "%CLIENTSAS%" "%CLIENTSSL%" %WAS_TRACE% %WAS_DEBUG% %CONSOLE_ENCODING% %USER_INSTALL_PROP% "-Dwas.install.root=%WAS_HOME%" "com.ibm.wsspi.bootstrap.WSPreLauncher" -nosplash -application "com.ibm.ws.bootstrap.WSLauncher"   "com.ibm.ws.admin.services.WsServerStop" "%CONFIG_ROOT%" "%WAS_CELL%" "%WAS_NODE%" "nodeagent" -nodeagent %*
SET RC=%ERRORLEVEL%
