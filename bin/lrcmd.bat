...
:runcmd
set CLASSPATH=%WAS_HOME%/runtimes/com.ibm.ws.webservices.thinclient_8.5.0.jar;%WAS_HOME%/plugins/com.ibm.ws.batch.runtime.jar

set PERFJAVAOPTION=-Xms256m -Xmx256m -Xj9 -Xquickstart
@echo off

"%JAVA_HOME%\bin\java" %PERFJAVAOPTION% %WAS_LOGGING% %javaoption% %CONSOLE_ENCODING% %WAS_DEBUG% "%CLIENTSOAP%" "%CLIENTSAS%" "%CLIENTSSL%" -Dcom.ibm.ws.container=standalone -Dcom.ibm.ws.management.standalone=true "-Duser.install.root=%USER_INSTALL_ROOT%" "-Dwas.install.root=%WAS_HOME%" "-Dcom.ibm.itp.location=%WAS_HOME%\bin" "-Dws.ext.dirs=%WAS_EXT_DIRS%" "-classpath" "%CLASSPATH%" com.ibm.ws.batch.wsbatch %*
