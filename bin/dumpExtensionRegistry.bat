...
@REM Extension Registry Dump Utility
...
"%JAVA_HOME%\bin\java" -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% %DEBUG% %WAS_TRACE% %CONSOLE_ENCODING% "%CLIENTSOAP%" "%JAASSOAP%" "%CLIENTSAS%" "%CLIENTSSL%" %USER_INSTALL_PROP% "-Dwas.install.root=%WAS_HOME%" com.ibm.wsspi.bootstrap.WSPreLauncher -nosplash -application "com.ibm.ws.bootstrap.WSLauncher" "com.ibm.ws.runtime.DumpExtensionRegistry" "%CONFIG_ROOT%" "%WAS_CELL%" "%WAS_NODE%" %*
set RC=%ERRORLEVEL%
