...
%JAVA_EXE% -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% "-Dwas.install.root=%WAS_HOME%" %WAS_LOGGING% %CONSOLE_ENCODING% "%CLIENTSAS%" -Dserver.root="%WAS_HOME%" com.ibm.ws.bootstrap.WSLauncher com.ibm.websphere.rastools.collector.Collector -InstallRoot "%WAS_HOME%" -JavaHome "%JAVA_HOME%" -ConfigRoot "%CONFIG_ROOT%" -WasNode "%WAS_NODE%" -CellName "%WAS_CELL%"  -ProfileRoot "%USER_INSTALL_ROOT%" %*
set RC=%ERRORLEVEL%
