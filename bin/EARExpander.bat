...
@REM usage:
@REM   EARExpander -ear <file-name>
@REM               -operation {expand | collapse}
@REM               -directory <directory> | -operationDir <directory>
@REM               [-expansionFlags {all | war}]
@REM               [-verbose]
@REM
@REM return codes:
@REM   -1  syntax error
...
"%JAVA_HOME%\bin\java.exe" -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% "-Dwas.install.root=%WAS_HOME%" "-Dcom.ibm.ffdc.log=%FFDCLOG%" %WAS_LOGGING% %CONSOLE_ENCODING% com.ibm.ws.bootstrap.WSLauncher com.ibm.websphere.management.application.commands.EARExpander %*
set RC=%ERRORLEVEL%
