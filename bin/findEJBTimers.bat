...
@REM EJB Timer Service Find EJB Timers Command Launcher

@REM Launch Arguments:
@REM
@REM findEJBTimers <server> <filter> [options]
@REM server  : the name of the server process to look for timers.
@REM filter: <-all> | <-timer timerid> | <-app [-mod [-bean]]>
@REM         -all                        : find all timers
@REM         -timer <timerid>            : find a specific timer only
@REM         -app <application name>     : find all timers for an applicaiton
@REM         -mod <module name>          : find all timers for a module
@REM         -bean <bean name>           : find all timers for an EJB type
@REM options:
...
@REM For debugging the find program itself
