...
if "%WAS_ENDORSED_DIRS%"=="" (
  %JAVA_EXE% %CONSOLE_ENCODING% -Dwas.install.root="%WAS_HOME%" -classpath "%WAS_CLASSPATH%" com.ibm.websphere.product.VersionInfo %*
) else (  
  %JAVA_EXE% -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" %CONSOLE_ENCODING% -Dwas.install.root="%WAS_HOME%" -classpath "%WAS_CLASSPATH%" com.ibm.websphere.product.VersionInfo %*
)
