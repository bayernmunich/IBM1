...
#
#  uninstall:
# wsadmin.sh -f installOIDCRP.py remove
#------------------------------------------------------------------------------
import sys
secObject = ""
webAuth = ""
...
#------------------------------------------------------------------------------
# Get the directory, as a string, of the WebSphereOIDCRP application.
#
# The WebSphereOIDCRP.ear file is located in the <WAS_HOME>/installableApps directory
#------------------------------------------------------------------------------
def getAppDir(cell, node):
	fileSep = getFileSep(node)
	return getinstallableAppsDir(cell, node) + fileSep + "WebSphereOIDCRP.ear"
...
#------------------------------------------------------------------------------
# deploy
# Deploy WebSphereOIDCRP.ear using the AdminApp install command.
#------------------------------------------------------------------------------
def deployOidcClientApplication(cell, node, server):
	appDir  = getAppDir(cell, node)
	try:
		print "Deploying WebSphereOIDCRP.ear"
		AdminApp.install(appDir, ['-node', node, '-server', server, '-appname', 'WebSphereOIDCRP', '-usedefaultbindings'])
		...
		if error.count("7279E") > 0: # catch WASX7279E (app with given name already exists)
			...
			print "Exception occurred during installOIDCRP(" + cell +", " + node + ", " + server + "):", error
		return 0
	return 1
...
#------------------------------------------------------------------------------
# deploy
# Deploy WebSphereOIDCRP.ear using the AdminApp install command in cluster.
#------------------------------------------------------------------------------
def deployOidcClientApplicationInCluster(cluster):
	appDir = java.lang.System.getenv('WAS_HOME') + "/installableApps/WebSphereOIDCRP.ear"

	try:
		print "Deploying WebSphereOIDCRP.ear"
		AdminApp.install(appDir, ['-cluster', cluster, '-appname', 'WebSphereOIDCRP', '-usedefaultbindings'])
...
#------------------------------------------------------------------------------
def printUsage():
	print "Usage: wsadmin -f installOIDCRP.py install appServer_NodeName appServer_ServerName"
	print "  or:  wsadmin -f installOIDCRP.py install clusterName"
	print "  or:  wsadmin -f installOIDCRP.py remove"
	print ""
...
def doInstall(nodeName,serverName):
	...
	if topology == None:
		...
		type = topology[3]
                ...
                retVal = deployOidcClientApplication(cell, node, server)
		if retVal == 1:
			AdminConfig.save()
		else:
...
#------------------------------------------------------------------------------
def doRemove():
	AdminApp.uninstall("WebSphereOIDCRP")
	AdminConfig.save()
...
def doClusterInstall(clusterName):
        retVal = deployOidcClientApplicationInCluster(clusterName)
	if retVal == 1:
		AdminConfig.save()
	else:
		print "Skipping Config Save"
...
if len(sys.argv) < 1 or len(sys.argv) > 3:
	...
	mode = sys.argv[0]
        if mode == "install":
		        ...
		        print "Installing OpenID Connect relying party..."
                ...
                print "Removing OpenID Connect relying party..."
		doRemove()
	else:
