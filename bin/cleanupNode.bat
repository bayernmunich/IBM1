...
"%JAVA_HOME%\bin\java" "-Djava.endorsed.dirs=%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% %WAS_DEBUG% %WAS_TRACE% %CONSOLE_ENCODING% %SSLCONFIG% "%CLIENTSOAP%" "%JAASSOAP%" "%CLIENTSAS%" "%CLIENTSSL%" %USER_INSTALL_PROP% "-Dwas.install.root=%WAS_HOME%" "com.ibm.ws.bootstrap.WSLauncher" "com.ibm.ws.management.tools.NodeCleanupUtility" "%CONFIG_ROOT%" "%WAS_CELL%" %*
set RC=%ERRORLEVEL%
