...
if exist "%JAVA_HOME%\bin\java.exe" (
   set JAVA_EXE="%JAVA_HOME%\bin\java" -Dws.ext.search.plugins.reverse="true"
) else (
...
"%JAVA_HOME%\bin\java" -Dws.ext.search.plugins.reverse="true" -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% %CONSOLE_ENCODING% -Dserver.root="%WAS_HOME%" %USER_INSTALL_PROP% "-Dwas.install.root=%WAS_HOME%" %WAS_LOGGING% com.ibm.ws.bootstrap.WSLauncher com.ibm.websphere.plugincfg.generator.PluginConfigGenerator "%WAS_HOME%" "%CONFIG_ROOT%" "%WAS_CELL%" "%WAS_NODE%" %*
set RC=%ERRORLEVEL%
