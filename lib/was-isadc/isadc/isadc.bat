...
if defined NEED_INIT (
	...
	"%JAVA_CMD%" %DEBUG_OPTS% -Xms64M -Xmx128M -cp %AUTOPD_CP% -Dautopd.home=%AUTOPD_HOME% -Djava.util.logging.config.file=%LOG_CONFIG% %CONSOLE_ENCODING% "%CLIENTSAS%" -Dwas.root=%WAS_HOME% -Dwas.install.root=%WAS_HOME% -Duser.install.root=%USER_INSTALL_ROOT% com.ibm.ws.bootstrap.WSLauncher com.ibm.autopd.internal.console.ISACollectorConsole %* -init
	if exist "%DATA_HOME%\tmp\collectorInit.bat" (
		call "%DATA_HOME%\tmp\collectorInit.bat"
		"%JAVA_CMD%" %DEBUG_OPTS% -Xms64M -Xmx128M -cp %AUTOPD_CP%;!COLLECTOR_CLASSPATH! -Dautopd.home=%AUTOPD_HOME% -Djava.util.logging.config.file=%LOG_CONFIG% %CONSOLE_ENCODING% "%CLIENTSAS%" -Dwas.root=%WAS_HOME% -Dwas.install.root=%WAS_HOME% -Duser.install.root=%USER_INSTALL_ROOT% com.ibm.ws.bootstrap.WSLauncher com.ibm.autopd.internal.console.ISACollectorConsole %*
	)
) else (
	"%JAVA_CMD%" %DEBUG_OPTS% -Xms64M -Xmx128M -cp %AUTOPD_CP% -Dautopd.home=%AUTOPD_HOME% -Djava.util.logging.config.file=%LOG_CONFIG% %CONSOLE_ENCODING% "%CLIENTSAS%" -Dwas.root=%WAS_HOME% -Dwas.install.root=%WAS_HOME% -Duser.install.root=%USER_INSTALL_ROOT% com.ibm.ws.bootstrap.WSLauncher com.ibm.autopd.internal.console.ISACollectorConsole %*
)
