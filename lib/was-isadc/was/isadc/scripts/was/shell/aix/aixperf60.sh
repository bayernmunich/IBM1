...
#
# ./aixperf60.sh [PID(s)_of_the_problematic_JVM(s)_separated_by_spaces]
#
...
echo "$0 : ./aixperf60.sh [PID(s)_of_the_problematic_JVM(s)_separated_by_spaces]"
exit 1
fi
##########################
# Create output files    #
...
do
	echo > dbxtrace.$i.out
	print $(date) "MustGather>>      dbxtrace.$i.out" | tee -a screen.out	
done
...
# Collect the dbxtrace: date at the top, data, and then a blank line
for i in $*
do
	print $(date) "MustGather>> Collecting the dbxtrace against PID $i..." | tee -a screen.out
	date >> dbxtrace.$i.out
	./*dbxtrace_aix.sh -a $i >> dbxtrace.$i.out 2>&1
	echo >> dbxtrace.$i.out
	print $(date) "MustGather>> Collection of dbxtrace against PID $i complete." | tee -a screen.out
done
