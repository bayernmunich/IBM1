...
# 12/27/2000 Modified to point to correct paths for tbrapp1 JPB
# --------------------------------------------------------
...
#
typeset PID
EXECUTABLE=/usr/java1.1/bin/sparc/native_threads/java
CORE_FILE=core
...
then
    ...
    typeset -r DBX_COMMAND="/opt/SUNWspro/bin/dbx $EXECUTABLE $CORE_FILE" 
    ...
    typeset -r DBX_COMMAND="/opt/SUNWspro/bin/dbx -a $PID"
...
nawk -v thread_num=$THREAD_NUM -v max_threads=$MAXTHREADS '
  BEGIN {
    print "print \"where :\""
    print "where"
    print "print \"regs -F :\""
    ...
    print "print \"-----------------------------------------------------------------\""
      print "thread -info t@" thread_num
      thread_num++
    }
  }
...
typeset -r REMOVE_END="/^['][\$]t[0-9]*['] is not an existing thread./,\$d"
typeset -r REMOVE_BAD_LINES="/^program is not active\$/d"

print "Running dbx..." 1>&2
$DBX_COMMAND < $COMMAND_FILE 2>&1 | sed -e "$REMOVE_END" -e "$REMOVE_BAD_LINES"
