...
<%
   ...
   if (formType != null && formType.length() > 0) {
		...
		if (index > 0) {
			String fType = formType.substring (index+1); // CreateDynamicClusterForm
			if (fType.endsWith (DETAILFORM)) {
				...
				fieldLevelHelpTopic = objectType + ".collection";
           		...
           		objectType = fType.substring (0, fType.length() - FORM.length()); //CreateDynamicCluster
           	 	fieldLevelHelpTopic = objectType; //CreateDynamicCluster
         	} else {
   ...
   //override the titleKey and descKey if we are using this wizard to add members instead of create dynamic cluster
   boolean isDynamicClusterMemberWizard = com.ibm.ws.console.dynamiccluster.utils.Utils.isDynamicClusterMemberWizard(session);
   if (isDynamicClusterMemberWizard) {
      titleKey = "dynamiccluster.addmember.title";
      descKey = "dynamiccluster.addmember.description";
   }
%>
...
<html:form action="<%=actionPath%>" name="<%=actionForm%>" styleId="<%=actionForm%>" type="<%=formType%>">
  ...
  <TR>
  <% if (!fieldLevelHelpTopic.equals("") && !helpPluginId.equals("")) { //There is an extra column in the title controls row%>
...
%>
        ...
        <%@ include file="/com.ibm.ws.console.dynamiccluster/helpPortlet.jspf" %>
