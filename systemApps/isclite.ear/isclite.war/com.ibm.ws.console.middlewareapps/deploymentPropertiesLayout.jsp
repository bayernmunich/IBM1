...
function addClicked(optionsToAdd) {
    if (optionsToAdd.length > 0) {
        ...
        if (optionsToAddString.length != 0) {
            ...
            window.location = encodeURI("/ibm/console/InstallWizDeployOptions.do?AddClicked=true" + "&moduleName=" + encodeURI(moduleName) + "&contextRoot=" + encodeURI(contextRoot) + "&selectedVirtualHost" + encodeURI(selectedVirtualHost) + "&TargetsToAdd=" + encodeURI(optionsToAddString)
...
function removeClicked(optionsToRemove) {
    if (optionsToRemove.length > 0) {
        ...
        if (optionsToRemoveString.length != 0) {
            ...
            window.location = encodeURI("/ibm/console/InstallWizDeployOptions.do?RemoveClicked=true" + "&moduleName=" + encodeURI(moduleName) + "&contextRoot=" + encodeURI(contextRoot) + "&selectedVirtualHost" + encodeURI(selectedVirtualHost) + "&TargetsToRemove=" + encodeURI(optionsToRemoveString)
               + "&csrfid=" + "<%=session.getAttribute("com.ibm.ws.console.CSRFToken")%>");
        }
    }
}
...
function filterTargets() {
	...
	if (filterValueString.length > 0) {
	    if (filterValueString == "Name") {
            ...
            window.location = encodeURI("/ibm/console/InstallWizDeployOptions.do?AddFilter=true" + "&NewFilterValue=" + encodeURI(filterValueString)
               + "&csrfid=" + "<%=session.getAttribute("com.ibm.ws.console.CSRFToken")%>");
        }
	}
}
...
function nameFilterClicked() {
    window.location = encodeURI("/ibm/console/InstallWizDeployOptions.do?AddNameFilter=true" + "&NewNameFilterValue=" + encodeURI(document.forms[0].nameFilter.value)
      + "&csrfid=" + "<%=session.getAttribute("com.ibm.ws.console.CSRFToken")%>");
}
...
<%
    ...
    InstallMiddlewareAppForm theForm = (InstallMiddlewareAppForm) session.getAttribute("InstallMiddlewareAppForm");
...
<table border="0" cellpadding="3" cellspacing="3" width="100%" role="presentation">
	...
	%>
	<input type="hidden" name="moduleName" value="module1"/>
    <% } else { %>
    <tr valign="top">
        <td class="table-text">
            ...
            <table border="0" cellspacing="0" role="presentation">
                ...
                <tr valign="top">
                    ...
                    <%-- <td class="table-text" id="NameFilterCell" style="<%=nameFilterDisplay%>">
                        <label title="<bean:message key="middlewareapps.deploy.filter.name.description"/>">
