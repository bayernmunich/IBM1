...
<SCRIPT>
var changing = false;
var selectedAvailableCL; // array of currently selected available nodes 
var selectedCurrentCL; // array of currently selected member nodes 
var availCL;
var currentCL;
var availField;
var outputField;
...
function initVars(formElem){
	...
	changing = false;
	selectedAvailableCL = new Array(); //associative array of currently selected available nodes (keyed on Node Name)
	selectedCurrentCL = new Array(); //associative array of currently selected member nodes (keyed on Node Name)
	...
	availCL = theform.elements[firstindex]; //leave 0 in case firstMembershipFormElemIndex is a string
	currentCL = theform.elements[3 + firstindex];
...
/*
This function sets the global selectedAvailableCL array to correspond with the selected 
objects in the box represented by "nodeList" (either currentCL or availCL)
*/
function setSelectedCL(nodeList, selectObject){
	changing = true;
	empty(nodeList);	
...
optionsToMove: one of either selectedCurrentCL or selectedAvailableCL 
from:  An options array (from either availCL or currentCL)
to:    An options array (from the other of either availCL or currentCL)
*/
...
function updateHiddenFields(){	
	var tempString = "";
	if (currentCL.options.length > 0){
		tempString += currentCL.options[0].value;
		for (var x = 1; x < currentCL.options.length; x++){
			tempString += "," + currentCL.options[x].value;
		}		
	}	
	outputField.value = tempString;	
	
	tempString = "";
	if (availCL.options.length > 0){
		tempString += availCL.options[0].value;
		for (var x = 1; x < availCL.options.length; x++){
			tempString += "," + availCL.options[x].value;
		}		
	}
...
function resetForm(){
	...
	setOptionsList(currentCL.options, originalOutputField);
	setOptionsList(availCL.options, originalAvailField);	
}
    ...
    <tbody>
        <tr>
  		<td class="table-text" nowrap valign="top" colspan="<%= numberOfColumns %>">
  			<label title='<bean:message key="healthclass.wizard.steps.clusters"/>'>
...
Collection availableCLBean = testForm.getAvailableCL();
Collection selectedCLBean = testForm.getSelectedCL();
String hcName = testForm.getName();
pageContext.setAttribute("availableCLBean", availableCLBean);
pageContext.setAttribute("selectedCLBean", selectedCLBean);

String availCLSelectChange = "initVars(this); setSelectedCL(selectedAvailableCL, availCL)";
String currCLSelectChange = "initVars(this);setSelectedCL(selectedCurrentCL, currentCL)";
String removeClicked = "initVars(this);moveOptions(selectedCurrentCL, currentCL.options, availCL.options)";
String addClicked = "initVars(this);moveOptions(selectedAvailableCL, availCL.options, currentCL.options)";
%>
...
<tr valign="top"> 
					...
					<tr>
						<td class="table-text">
							<table>
								...
								<tr>
									<td class="table-text">							
									  	<html:select property="notUsed" multiple="multiple" size="10" onchange="initVars(this); setSelectedCL(selectedAvailableCL, availCL)" style="width:200px;">
                        					<html:options name="availableCLBean" />
                						</html:select>
									</td>
		                			...
		                			<td class="table-text"  align="center" valign="top" width="35%">
		 		                		<html:select property="notUsed" multiple="multiple" size="10" onchange="initVars(this, 2);setSelectedCL(selectedCurrentCL, currentCL)" style="width:200px;">
                        					<html:options name="selectedCLBean" />
                						</html:select>
		               		 		</td>
		       					</tr>
		 					</table>
		...
		<html:hidden property="availableCLString"/>
        <html:hidden property="selectedCLString"/>
