...
<html:form action="<%=formAction%>" name="<%=formName%>" styleId="<%=formName%>" type="<%=formType%>">
        ...
        <tiles:insert definition="<%=buttons%>" flush="true"/>
			...
			<fieldset style="border:0px; padding:0pt; margin: 0pt;">
       			<legend class="hidden" ><bean:message key="healthClasses" /></legend>
            ...
            <logic:iterate id="listcheckbox" name="<%=iterationName%>" property="<%=iterationProperty%>">
                <bean:define id="hcName" name="listcheckbox" property="name" />
                <%
                   String description = "";
                   String reactionMode = "";
                   try {
                      com.ibm.ws.sm.workspace.RepositoryContext cellContext = (com.ibm.ws.sm.workspace.RepositoryContext) session.getAttribute(com.ibm.ws.console.core.Constants.CURRENTCELLCTXT_KEY);
                      com.ibm.ws.sm.workspace.RepositoryContext hcContext = cellContext.findContext("healthclasses/" + hcName);
                      org.eclipse.emf.ecore.resource.Resource r = hcContext.getResourceSet().createResource(org.eclipse.emf.common.util.URI.createURI("healthclass.xml"));
                      r.load(new java.util.HashMap());
                      com.ibm.websphere.models.config.healthpolicy.HealthClass hc = (com.ibm.websphere.models.config.healthpolicy.HealthClass) r.getContents().get(0);
                      ...
                      org.apache.struts.util.MessageResources messages = (org.apache.struts.util.MessageResources) getServletContext().getAttribute(org.apache.struts.Globals.MESSAGES_KEY);
                      reactionMode = messages.getMessage(request.getLocale(), com.ibm.ws.console.healthconfig.util.HealthConfigConverter.reactionModeIntToString(hc.getReactionMode()));
                   }
                   catch (Exception ex) {
                   }
                %>
...
<%                       } else {
                     	...
                     	<%  } else if (columnField.equalsIgnoreCase("name")) { %>
                       	...
                       	<%  } else if (columnField.equalsIgnoreCase("reactionMode")) { // reactionMode
                               %>
                                        <%=reactionMode%>
                       	<%  } else if (columnField.equalsIgnoreCase("description")) { // description
                               %>
                                        <%=description%>
                       	<%  }
