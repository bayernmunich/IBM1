...
<SCRIPT>
var changing = false;
var selectedAvailableAP; // array of currently selected available nodes 
var selectedCurrentAP; // array of currently selected member nodes 
var availAP;
var currentAP;
var availField;
var outputField;
...
function initVars(formElem){
	...
	changing = false;
	selectedAvailableAP = new Array(); //associative array of currently selected available nodes (keyed on Node Name)
	selectedCurrentAP = new Array(); //associative array of currently selected member nodes (keyed on Node Name)
	...
	availAP = theform.elements[firstindex]; //leave 0 in case firstMembershipFormElemIndex is a string
	currentAP = theform.elements[3 + firstindex];
...
/*
This function sets the global selectedAvailableAP array to correspond with the selected 
objects in the box represented by "nodeList" (either currentAP or availAP)
*/
function setSelectedAP(nodeList, selectObject){
	changing = true;
	empty(nodeList);	
...
optionsToMove: one of either selectedCurrentAP or selectedAvailableAP 
from:  An options array (from either availAP or currentAP)
to:    An options array (from the other of either availAP or currentAP)
*/
...
function updateHiddenFields(){	
	var tempString = "";
	if (currentAP.options.length > 0){
		tempString += currentAP.options[0].value;
		for (var x = 1; x < currentAP.options.length; x++){
			tempString += "," + currentAP.options[x].value;
		}		
	}	
	outputField.value = tempString;	
	
	tempString = "";
	if (availAP.options.length > 0){
		tempString += availAP.options[0].value;
		for (var x = 1; x < availAP.options.length; x++){
...
function resetForm(){
	...
	setOptionsList(currentAP.options, originalOutputField);
	setOptionsList(availAP.options, originalAvailField);	
}
    ...
    <tbody>
        <tr>
  		<td class="table-text" nowrap valign="top" colspan="<%= numberOfColumns %>">
  			<label title='<bean:message key="healthclass.wizard.steps.appservers"/>'>
...
Collection availableAPBean = testForm.getAvailableAP();
Collection selectedAPBean = testForm.getSelectedAP();
String hcName = testForm.getName();
pageContext.setAttribute("availableAPBean", availableAPBean);
pageContext.setAttribute("selectedAPBean", selectedAPBean);

String availAPSelectChange = "initVars(this); setSelectedAP(selectedAvailableAP, availAP)";
String currAPSelectChange = "initVars(this);setSelectedAP(selectedCurrentAP, currentAP)";
String removeClicked = "initVars(this);moveOptions(selectedCurrentAP, currentAP.options, availAP.options)";
String addClicked = "initVars(this);moveOptions(selectedAvailableAP, availAP.options, currentAP.options)";
%>
					...
					<tr>
						<td class="table-text">
							<table>
								...
								<tr>
									<td class="table-text">							
									  	<html:select property="notUsed" multiple="multiple" size="10" onchange="initVars(this); setSelectedAP(selectedAvailableAP, availAP)" style="width:200px;">
		                        			<html:options name="availableAPBean" />
		                				</html:select>
									</td>
		                			...
		                			<td class="table-text"  align="center" valign="top" width="35%">
		 		                		<html:select property="notUsed" multiple="multiple" size="10" onchange="initVars(this, 2);setSelectedAP(selectedCurrentAP, currentAP)" style="width:200px;">
                        					<html:options name="selectedAPBean" />
                						</html:select>
		               		 		</td>
		       					</tr>
		 					</table>
		...
		<html:hidden property="availableAPString"/>
        <html:hidden property="selectedAPString"/>
	</tbody>
</table>
