    ...
    <%}%>
            ...
            <% if ((isReadOnly.equalsIgnoreCase("yes") || (isReadOnly.equalsIgnoreCase("true")))) { 
               ...
               <input TYPE="text" id="<%=property%>" class="textEntryReadOnly" value='<bean:write property="<%=property %>" name="<%=formBean%>"/>' DISABLED/> --%>
                ...
                <html:text readonly="true" property="<%=propName%>" name="<%=formBean%>"  value="<%=propValue%>" size="<%=size%>" styleClass="textEntryReadOnly" styleId="<%=property%>" title="<%=messages.getMessage(request.getLocale(),desc)%>"/><%      
          ...
          <% 		}
                ...
                if (isRequired.equalsIgnoreCase("yes")) {
            %>
                <html:text property="<%=propName%>" name="<%=formBean%>" value="<%=propValue%>" size="<%=size%>" styleClass="textEntryRequired" styleId="<%=property%>" title="<%=messages.getMessage(request.getLocale(),desc)%>"/><%      
                }  else {
                %><html:text property="<%=propName%>" name="<%=formBean%>" value="<%=propValue%>" size="<%=size%>" styleClass="textEntry" styleId="<%=property%>" title="<%=messages.getMessage(request.getLocale(),desc)%>"/><%
              	}
         		if (units != null && !units.equals(" ") && !units.equals("")) { %>&nbsp;<bean:message key="<%=units%>"/> <% }
