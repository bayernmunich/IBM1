...
<%
boolean isEnableResponseTimeGoals = false;
String enableResponseTimeGoals = XDOperationsViewConfigHelper.getConfigProperty(XDOperationsViewConfigHelper.RESPONSE_TIME_GOALS_DISABLE);
	...
	if (enableResponseTimeGoals.equalsIgnoreCase("true")) 
		isEnableResponseTimeGoals = true;
            ...
            <% } else if (item.getType().equalsIgnoreCase("jsp")) { %>
                ...
                <% } else if (item.getType().equalsIgnoreCase("Select")) { 
                    ...
                    while(st.hasMoreTokens()) 
                    {
						...
						//special case - if the Bus Grid package is not installed, we don't want to show BG goals.
						if (item.getAttribute().equals("goalType")){
							if (!isEnableResponseTimeGoals) {
								if (!com.ibm.ws.xd.util.XD.isEnabledCG()){
	                                if (str.equals("GOAL_TYPE_QUEUETIME") || str.equals("GOAL_TYPE_COMPLETIONTIME"))
										continue;
	                            }
							}
							else {
								valueVector.addElement("GOAL_TYPE_IMPORTANCE");
								break;
							}
						}
