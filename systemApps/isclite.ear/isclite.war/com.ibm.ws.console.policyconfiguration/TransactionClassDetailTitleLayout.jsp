...
<%@ page language="java" import="com.ibm.ws.console.core.form.AbstractDetailForm,java.util.regex.Pattern,org.apache.struts.util.MessageResources,org.apache.struts.action.Action"%>
...
//This is quite a little hack.  We need to get the breadcrumbs correct, and we don't get the 
//same prior page back from IE and Mozilla.  While mozilla gets the right page, the contextId is
//incorrect...so for this page, just put it the way we want it.
                ...
                //} else {
                   ...
                   if (request.getParameter("refId") == null) {
                        AbstractDetailForm pForm = (AbstractDetailForm)session.getAttribute("ParentForm");
                        refId = "&refId="+pForm.getRefId();
                         //if (priorpage.indexOf("/secure/logon.do") > -1) {
                                //this is for IE mainly
                                priorpage = priorpage.substring(0,priorpage.indexOf("/ibm/console/"));
                                priorpage = priorpage + "/ibm/console/DisplayServiceClassDetails.do";
                                altprior = priorpage + "?perspective=tab.configuration"+ refId + contextId + resourceUri + aperspective;
                        //}
                        //else altprior = priorpage;
//                   }
                }
        }
        ...
        // Defect 230092 - check if the priorpage was cheatSheet
        int isCheatSheet = priorpage.indexOf("cheatSheet");

        // for IE, if the prior page was null, set isCheatSheet to a non negative number, to disable breadcrumbs
        if (request.getHeader("Referer") == null){
		isCheatSheet = 1;
	  }
        // Defect 230092
        ...
        // If the referer is navigator||banner||logoff||clustertree||cheatsheet, then zero out arrays - leave logon off detail title
        if ((isnavigator > -1) || (isbanner > -1) || (islogoff > -1) || (isclustertree > -1) || (isCheatSheet > -1)) {
