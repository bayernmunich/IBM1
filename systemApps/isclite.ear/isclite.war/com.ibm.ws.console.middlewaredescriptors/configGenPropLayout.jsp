...
<%@ page import="com.ibm.ws.console.middlewaredescriptors.form.MiddlewareDescriptorDetailForm, com.ibm.ws.console.core.item.PropertyItem"%>
...
<%  
MiddlewareDescriptorDetailForm formDescriptor = (MiddlewareDescriptorDetailForm)session.getAttribute(formName);
boolean isSupportedDescriptor = formDescriptor.isDiscoverySupported();
int discoveryInterval = formDescriptor.getDiscoveryInterval();
String renderReadOnlyView = "no";
if ( ((readOnlyView != null) && (readOnlyView.equalsIgnoreCase("yes")) ) ||((isSupportedDescriptor == false) && (discoveryInterval != -1))) {
...
<table class="framing-table" border="0" cellpadding="5" cellspacing="0" width="100%" role="presentation" >
    ...
    <tr valign="top">
        ...
        <tiles:insert page="/com.ibm.ws.console.middlewaredescriptors/dynamicSelectionLayout.jsp" flush="false">
        <tiles:put name="property" value="<%=item.getAttribute()%>" />
        <tiles:put name="readOnly" value="true" />
        <tiles:put name="isRequired" value="<%=item.getRequired()%>" />
        <tiles:put name="label" value="<%=item.getLabel()%>" />
...
<table class="framing-table" border="0" cellpadding="5" cellspacing="0" width="100%" role="presentation" >
    ...
    <%
      MiddlewareDescriptorDetailForm form = (MiddlewareDescriptorDetailForm)session.getAttribute(formName);
      boolean isSupported = form.isDiscoverySupported();
      int discoveryVal = form.getDiscoveryInterval();
      if (showButtons!=null && showButtons.equalsIgnoreCase("yes") && ((isSupported == true) || (discoveryVal == -1)))
        { %>
...
<br>
<html:form action="<%=formAction%>" name="<%=formName%>" type="<%=formType%>" onreset="<%=onReset%>">
 			<tiles:insert page="/com.ibm.ws.console.middlewaredescriptors/versionDescriptorCollectionTableLayout.jsp" flush="true">
