    ...
    <%}%>
            ...
            <% if (isRequired.equalsIgnoreCase("yes")) { %>
              <span class="requiredField">
                ...
                <% }else{ %>
                <img id="requiredImage" src="<%=request.getContextPath()%>/images/attend.gif" width="8" height="8" align="absmiddle" alt="<bean:message key="information.required"/>" title="<bean:message key="information.required"/>">
                <bean:message key="<%=label%>" />
                <% } %>
                </label>
              </span>
            ...
            <% if ((isReadOnly.equalsIgnoreCase("yes") || (isReadOnly.equalsIgnoreCase("true")))) { 
               ...
               <input TYPE="text" id="<%=property%>" class="textEntryReadOnly" value='<bean:write property="<%=property %>" name="<%=formBean%>"/>' DISABLED/> --%>
                ...
                <html:text readonly="true" property="<%=propName%>" name="<%=formBean%>"  value="<%=propValue%>" size="<%=size%>" styleClass="textEntryReadOnly" styleId="<%=propName%>" title="<%=messages.getMessage(request.getLocale(),desc)%>"/><%      
        		...
        		if (units != null && !units.equals(" ") && !units.equals("")) { %>&nbsp;<bean:message key="<%=units%>"/> <% }
		                ...
		                <%-- Need to include some element/style to respect whitespaces in names  
		               	<input TYPE="text" id="<%=property%>" class="textEntryReadOnly" value='<bean:write property="<%=property %>" name="<%=formBean%>"/>' DISABLED/> --%>
		                <%--<html:text property="<%=property%>" name="<%=formBean%>" size="<%=size%>" styleClass="textEntryReadOnly" styleId="<%=property%>" onkeydown="captureRO(this)" onkeyup="returnRO(this)"/>--%>
			            <table border="0" cellpadding="0" cellspacing="0" width="100%" role="presentation"><tr><td class="table-text" valign="top">
			            <DIV CLASS="readOnlyElement" ID="<%=propName%>">
          ...
          <% 		}
                ...
                if (isRequired.equalsIgnoreCase("yes")) {
            %>
                <html:text property="<%=propName%>" name="<%=formBean%>" value="<%=propValue%>" size="<%=size%>" styleClass="textEntryRequired" styleId="<%=propName%>" title="<%=messages.getMessage(request.getLocale(),desc)%>"/><%      
                }  else {
                %><html:text property="<%=propName%>" name="<%=formBean%>" value="<%=propValue%>" size="<%=size%>" styleClass="textEntry" styleId="<%=propName%>" title="<%=messages.getMessage(request.getLocale(),desc)%>"/><%
              	}
         		if (units != null && !units.equals(" ") && !units.equals("")) { %>&nbsp;<bean:message key="<%=units%>"/> <% }
