...
<%                      } else if(columnField.equalsIgnoreCase("state")) { 
							...
							<img id="<bean:message key='<%=state%>'/>Img_<%=chkcounter %>" 
								src="<%=request.getContextPath()+iconPath%>"
								title="<bean:message key='<%=state%>'/>" 
...
<%                      } else if(columnField.equalsIgnoreCase("stability")) { 
							...
							<% if (stability.equals(com.ibm.ws.console.xdoperations.util.Constants.STABILITY_STABLE)) {
									iconPath = com.ibm.ws.console.xdcore.util.XDCoreConstants.STABILITY_LEVEL_STABLE;
							   } else if (stability.equals(com.ibm.ws.console.xdoperations.util.Constants.STABILITY_QUESTIONABLE)) {
									iconPath = com.ibm.ws.console.xdcore.util.XDCoreConstants.STABILITY_LEVEL_QUESTIONABLE;
							   } else if (stability.equals(com.ibm.ws.console.xdoperations.util.Constants.STABILITY_UNSTABLE)) {
							...
							%>
							<img id="<bean:message key='<%=stability%>'/>Img_<%=chkcounter %>" 
								src="<%=request.getContextPath()+iconPath%>"
								title="<bean:message key='<%=stability%>'/>" 
...
<% } catch (Exception e) {
    ...
    <%
        ...
        if (chkcounter == 0) {
           out.println("<table class='framing-table' cellpadding='3' cellspacing='1' width='100%' role='presentation'>");
