...
if(httpAction != null) {
	actionHandler = "RulesCollectionForm";
			   ...
			   <table cellpadding="5" cellspacing="0" role="presentation">
				 <tr>
				   <td class="table-text" valign="top">
				     <%
                        String ruleBuilderLayout = "/com.ibm.ws.console.xdcore/ruleBuilderLayout.jsp";
                        ...
                        if (label.equals("routing.rules.policy.expression")) {
                        	...
                        	quickHelpTopic = "http_subexpression.html";
                        	quickPluginId = "com.ibm.ws.console.rules";
                        }
				 ...
				 <tr>
				   <td class="table-text-white" valign="top">
			         <% String styleId = rule+rowindex; 
			         	...
			         	if(rule.contains("defaultHttpRoutingMatchExpression")){
			         %>
				 ...
				 <tr>
			       <td class="table-text-white" valign="top">
					 <html:textarea property="<%=rule%>" name="com.ibm.ws.console.rules.form.RulesCollectionForm" cols="60" rows="3" styleClass="textEntry" styleId="<%=styleId%>">
					 </html:textarea>
				   </td>
				 </tr>
				 ...
				 <tr>
			      ...
			      <% }
		            ...
		            editClickString = patternClick;
			     	...
			     	String other4 = "other4" + refId;
			     	String other5 = "other5" + refId;
				 %>				
				 ...
				 <tr id="ruleAction<%=refId%>">
				   <td class="table-text-white" valign="top" colspan=2>
                     <tiles:insert page="/com.ibm.ws.console.xdcore/httpRuleActionLayout.jsp" flush="false">
                 ...
                 <tr>
            	   <td class="table-text" valign="top" colspan=2>
                     <span style="margin-left:1em">
				       ...
				       <html:submit styleClass="buttons" property="rule.action.matchrule.validate.button" styleId="<%=other4 %>" onclick="<%=setRowIndex%>">
				         <bean:message key="rule.action.matchrule.validate.button" />
				       </html:submit>
                       <input type="button" name="Cancel" value="<bean:message key="button.cancel"  />" class="buttons" id="<%=other5 %>" onclick="showSectionParent('<%=rowindex%>', '<%=refId%>')"/>
                     </span>
                   </td>
				 </tr>
				 <% } %>
...
<%
			     if (!SecurityContext.isSecurityEnabled() ||
				     (adminAuthorizer.checkAccess(contextUri, "administrator") || adminAuthorizer.checkAccess(contextUri, "configurator"))) {
%>
    		       [ <a id="quickEditLink<%=rowindex%>" href="javascript:<%=editClickString%>;showSectionParent('<%=rowindex%>', '<%=refId%>')">
...
<%			     } %>
			...
			<% } else { %>
	            ...
	            <%
			      ...
			      if (template.equals("http.rules.routing")) {
			      	labelkey = labelkey.replace("http.rules.routing","routing");
			      	descriptionkey = descriptionkey.replace("http.rules.routing","routing");
			      }
	            %>
				...
				<%  if (template.equals("routing")) { %>
				    ...
				    } else if (template.equals("http.rules.routing")) { %>
	                    ...
	                    policyType = matchActionItems[0].toLowerCase();
	                    if (policyType.equals("permitmm")) {
                            policyType = "mm";
                        } else if (policyType.equals("permitstickymm")) {
                            policyType = "stickymm";
                        }
                        routingAction = "httprules.routing.label." + policyType;
                        
                        if (policyType.equalsIgnoreCase("redirect") || policyType.equalsIgnoreCase("localresource")) {
                        	input = "<br />";
							if(matchActionItems.length == 2)
								input += matchActionItems[1];
							else if(matchActionItems.length == 3)
								input += matchActionItems[1] + ":" + matchActionItems[2];
                        } else if (policyType.equalsIgnoreCase("reject")) {
                        	input = " " + matchActionItems[1];
                        } else if (policyType.equalsIgnoreCase("permit") || policyType.equalsIgnoreCase("mm") || 
                        		   policyType.equalsIgnoreCase("permitsticy") || policyType.equalsIgnoreCase("stickymm")) {
