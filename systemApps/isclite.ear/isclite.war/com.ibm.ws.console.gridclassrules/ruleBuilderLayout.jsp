...
<%@ page import="com.ibm.ws.console.core.form.RuleDetailForm"%>
...
<%@ page import="com.ibm.wsspi.batch.expr.core.*"%>
<%-- @ page import="com.ibm.wsspi.odr.OdrManagerFactory.*" --%>
<%@ page import="com.ibm.wsspi.batch.expr.Language.*"%>
...
function operandNameChange<%=refId%>(rowindex) {
	...
	window.location = "/ibm/console/<%=actionHandler%>.do?operand=" + selectedOperand + "&operator=" + selectedOperator + "&appendWith=" + appendWith + "&rule=" + rule + "&refId=<%=refId%>&csrfid=" + ConfigFileHelper.getCSRFId(request);
	//window.location = "/ibm/console/<%=actionHandler%>.do?operand=" + selectedOperand + "&operator=" + selectedOperator + "&appendWith=" + appendWith + "&rule=" + rule + "&refId=<%=refId%>";
}
...
function customOperandNameChange<%=refId%>(rowindex) {
	...
	window.location = "/ibm/console/<%=actionHandler%>.do?operand=" + selectedOperand + "&builder=" + builder + "&operator=" + selectedOperator + "&appendWith=" + appendWith + "&rule=" + rule + "&refId=<%=refId%>&csrfid=" + ConfigFileHelper.getCSRFId(request);
	//window.location = "/ibm/console/<%=actionHandler%>.do?operand=" + selectedOperand + "&builder=" + builder + "&operator=" + selectedOperator + "&appendWith=" + appendWith + "&rule=" + rule + "&refId=<%=refId%>";
}
...
<table cellpadding="0" cellspacing="0" border="0" class="wasPortlet">
  <tr>
    <td class="wpsPortletTitle">
      <!--<label for="rule.edit.link.subexpression" title="<bean:message key="rule.edit.link.subexpression"/>">-->
    ...
    <td class="wpsPortletTitleControls">
		<% if (quickHelpTopic !=null && quickHelpTopic.length()>0 && quickPluginId !=null && quickPluginId.length()>0) {%>
          <a href="/ibm/help/index.jsp?topic=/<%=quickPluginId%>/ae/<%=quickHelpTopic%>" target="_blank">
              <img id="<bean:message key="menu.help"/>"
              	...
              	border="0" align="absmiddle" tabindex="1"
  ...
  <tr>
    <td class="wpsPortletArea" colspan="3">
		      ...
		      <fieldset>
				...
				<table role="presentation">
				  <tbody>
					<tr>
					 <td class="table-text">
					  <label title="<bean:message key="rule.builder.operand.label.desc"/>">
		              ...
		              %>
						...
						<label class="hidden" for="<%=styleId%>"><bean:message key="none.text"/></label>
		...
		%>			
		           ...
		           // System.out.println("made it here 5a");
					...
					//	 System.out.println("made it here 5a1");
				        ...
				        <label class="hidden" for="<%=id%>"><bean:message key="<%=desc%>"/></label>
						<html:text property="<%=appendValueName%>"
							name="<%=formAction%>"
							size="<%=fieldSize%>"
							styleId="<%=id%>"
		...
		<%          if (!(co instanceof BooleanOperand)) {%>
					<tr>
		<%			
						com.ibm.wsspi.batch.expr.core.Identifier[] operators ;
						//if ( (customOperands != null) || (searchCustomOperands == true) ) {
							//operators = SIPRulesConsoleUtils.getClusterLangOperators(co);
						//	com.ibm.wsspi.batch.expr.Language cDefLang = com.ibm.wsspi.odr.OdrManagerFactory.getManager().getClusterDefinitionLanguage();
		...
		<%			    } else{ %>    			
							<tiles:insert page="/secure/layouts/textFieldLayout.jsp" flush="false">
								...
								<tiles:put name="idSubscript" value="<%=refId%>"/>
							</tiles:insert>
		<%			    } %>
		    			</tr>
					  </tbody>		
		...
		<%      			} %>
					...
					<tbody id="<%=inMatchInput%>">
		    		  <tr>
					    <td class="table-text">
			    		  <tiles:insert page="/com.ibm.ws.console.gridclassrules/inInputLayout.jsp" flush="false">
					    ...
					    <td valign="top" class="wizard-step-text" colspan="2">
		                  <%
		                     ...
		                     if(!isSingleRule){
		                  	...
		                  	if (customOperands != null) {  %>
		                  		 <html:submit property="installAction3" styleId="otherGen" styleClass="buttons" onclick="<%=setCustomRowIndex%>">
		              	    		<bean:message key="rule.builder.generate.button"/>
		 	  	          		</html:submit>
		                  <%} else {   %>
		                  		 <html:submit property="installAction" styleId="otherGen" styleClass="buttons" onclick="<%=setRowIndex%>">
		              	    		<bean:message key="rule.builder.generate.button"/>
		 	  	          		</html:submit>
					  ...
					  <tr>
					  	...
					  	<% if (customOperands != null) {
					  	 %>
			            	  ...
			            	  <html:submit property="installAction2" styleId="otherAppend" styleClass="buttons" onclick="<%=setCustomRowIndex%>">
						...
						%>
							<td valign="top" class="wizard-step-text">
			            	  <html:submit property="installAction" styleId="otherAppend" styleClass="buttons" onclick="<%=setRowIndex%>">
			              	  	<bean:message key="rule.builder.append.button"/>
			 	  	          </html:submit>
							</td>
						<% } %>
