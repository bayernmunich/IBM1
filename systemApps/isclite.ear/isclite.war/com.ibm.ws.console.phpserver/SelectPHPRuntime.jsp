...
<fieldset style="border:0px; padding:0pt; margin: 0pt;">
	<legend class="hidden"><bean:message key="phpserver.new.step3"/></legend>
...
<logic:iterate id="temp1" name="<%=actionForm%>" property="listPHPRuntime" indexId="idx">
<% 
SelectPHPRuntimeForm phpRuntimeForm = (SelectPHPRuntimeForm) temp1; 
String namePHP  = phpRuntimeForm.getNamePHP();   
String phpVersion  = phpRuntimeForm.getPHPVersion();
String phpServerLocation = phpRuntimeForm.getPHPLocation();  
String displayName = phpRuntimeForm.getDisplayName();
String displayVersion = phpRuntimeForm.getDisplayVersion();
...
<label class="collectionLabel" for="<%=radioId%>" title="<bean:message key="select.text"/> <%=displayName%>">
	<html:radio name="<%=actionForm%>" property="namePHP" value="<%=displayName%>" styleId="<%=radioId %>"/>
</label>
</td>
<td>
<%=displayName %>
