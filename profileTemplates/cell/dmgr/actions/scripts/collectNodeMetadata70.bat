...
"%JAVA_HOME%\jre\bin\java" %JAVA_DEBUG% -Dcmd.properties.file=%TMPJAVAPROPFILE% %CONSOLE_ENCODING% "%CLIENTSOAP%" "%CLIENTSAS%" "-Dtrace=%TRACE%" "-DtraceFile=%TRACEFILE%" "-Dserver.root=%WAS_HOME%" "-Dwas.install.root=%WAS_HOME%" "-Duser.install.root=%USER_INSTALL_ROOT%" com.ibm.wsspi.bootstrap.WSPreLauncher -nosplash -debug -application com.ibm.ws.bootstrap.WSLauncher com.ibm.ws.admin.core.CollectManagedObjectMetadata
set RC=%ERRORLEVEL%
